...
if(isset($_POST['searchbtn'])){ 
    ...
    $search = $_POST['search'];
        ...
        OR  stuNme  LIKE '%".$search."%' 
        OR  stuSurnme  LIKE '%".$search."%' ";     
...
echo "<h2>ข้อมูลนักศึกษา ภาควิชาคณิตศาสตร์และวิทยาการคอมพิวเตอร์</h2>";
echo "<br>";

echo "<form action='showStudent.php' method='post'>";
echo "<div class='row'>";
...
echo "</tr>";
    ...
    while($row = $result->fetch_assoc()) {
		...
		echo "<input type='hidden' name ='stuID'  value = '".$row["stuID"]."'/>";		
		echo "<input name = 'editStudent' type='submit' value='แก้ไขข้อมูล' />";
		echo "</form>";
		echo "</td>";
		...
		echo "<input type='hidden' name ='stuID'  value = '".$row["stuID"]."'/>";
		echo "<input name = 'delStudent' type='submit' value='ลบข้อมูล' />";
		echo "</form>";
		echo "</td>";
...
echo "<tr style = 'text-align: center;'><td style = 'text-align: center;'>";
	echo"<form name='divide' action = 'showStudent.php' method ='post'> ";
